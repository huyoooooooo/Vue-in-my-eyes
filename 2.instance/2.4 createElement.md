...
二、VNode 的创建
对 `tag` 进行一个判断，`tag` 可能是一个字符串也可能直接是一个组件。当是字符串时: 如果是原生标签，就直接创建 VNode； 如果是注册过的组件，通过 `createComponent` 创建 VNode；都不是则创建一个未知标签的 VNode。如果不是字符串时组件，调用 `createComponent` 生成 VNode。最终将 VNode 返回。（组件创建 VNode 在 3.x 章中学习）