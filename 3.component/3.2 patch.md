...
export function initInternalComponent (vm: Component, options: InternalComponentOptions) {
  ...
  opts.parent = options.parent  // 存储父组件实例
...
将实例 `$options` 原型指向 `vm.constructor` —— 组件构造器 Sub 的 `options` 属性。新的实例化过程存储父组件实例，存储占位符节点作为父节点。
...
export function initLifecycle (vm: Component) {
  ...
  if (parent && !options.abstract) {
    ...
    parent.$children.push(vm)   // 将当前组件实例压入父组件 $children 栈中-------
  }                             //                                            |  组件实例之间的父子关系确立
  vm.$parent = parent           // 实例上挂载父组件实例   ----------------------
