...
export function createComponent (
  ...
  const vnode = new VNode(    // 创建 vnode
...
通过 `extend` 方法，将每一个父组件都转变成构造器（类）。通过 `installComponentHooks` 给组件注册钩子函数。最终根据创建父组件的 vnode —— `tag` 是根据组件自定义名称或标签名创建，`children`、`elm` 为空，`context` 为当前组件实例。该父组件可能存在属性、事件、子节点等和构造器一并存储到对象中 —— `componentOptions` 内。
